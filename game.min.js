...
cell.push(entity);}}}}}});ig.Game.SORT={Z_INDEX:function(a,b){return a.zIndex-b.zIndex;},POS_X:function(a,b){return(a.pos.x+a.size.x)-(b.pos.x+b.size.x);},POS_Y:function(a,b){return(a.pos.y+a.size.y)-(b.pos.y+b.size.y);}};});Array.prototype.first=function(){return this[0];};Array.prototype.last=function(){return this[this.length-1];};

// lib/plugins/scene_manager.js
ig.baked=true;ig.module('plugins.scene_manager').requires('impact.game').defines(function(){Scene=ig.Game.extend({title:null,sceneManager:null,font:null,center:{x:0,y:0},init:function(title){this.title=title||null;this.center.x=ig.system.width*0.5;this.center.y=ig.system.height*0.5;},update:function(){this.parent();ig.game.collisionMap=this.collisionMap;ig.game._deferredKill=this._deferredKill;},draw:function(){this.parent();if(this.title!=null){if(this.font==null){this.font=new ig.Font('media/04b03.font.png');}
...
// lib/game/menu_scene.js
ig.baked=true;ig.module("game.menu_scene").requires("impact.game","impact.font","plugins.scene_manager").defines(function(){return window.MenuScene=Scene.extend({title_font:new ig.Font("media/fonts/existence_light_42_2.png"),font:new ig.Font("media/fonts/existence_light_16.png"),init:function(manager){this.manager=manager;return this.bindKeys();},bindKeys:function(){return ig.input.bind(ig.KEY.ENTER,'start');},update:function(){this.parent();if(ig.input.pressed('start')){return this.manager.startGame();}},draw:function(){var center_x,center_y,intro_text,text,top_y,x;this.parent();center_x=ig.system.width/2;center_y=ig.system.height/2;top_y=40;text="Boogie Woogie\n";this.title_font.draw(text,center_x,top_y,ig.Font.ALIGN.CENTER);intro_text="Move by holding the arrow keys.\n\n";intro_text+="Get points by collecting squares of your color.\n\n";intro_text+="Avoid squares that aren't your color.\n\n";intro_text+="When you collect all of your current color,\n";intro_text+="you switch to the most common color on the board.\n\n";intro_text+="Ready? Good luck!\n\n";intro_text+="Press Enter to start...";x=(ig.system.width-this.font.widthForString(intro_text))/2;return this.font.draw(intro_text,x,top_y*4);}});});window.getRandomInt=function(min,max){return Math.floor(Math.random()*(max-min+1))+min;};

// lib/game/imageblender.js
ig.baked=true;ig.module('game.imageblender').requires('impact.image').defines(function(){ig.Image.inject({onload:function(event){this.parent(event);var hashIndex=this.path.indexOf("#");if(hashIndex!==-1){this.convertToCanvas();var ctx=this.data.getContext("2d");var imgData=ctx.getImageData(0,0,this.data.width,this.data.height);var src=imgData.data;var sA,dA=1,len=src.length;var sRA,sGA,sBA,dA2,demultiply;var dRA=parseInt(this.path.substr(hashIndex+1,2),16)/255;var dGA=parseInt(this.path.substr(hashIndex+3,2),16)/255;var dBA=parseInt(this.path.substr(hashIndex+5,2),16)/255;for(var px=0;px<len;px+=4){sA=src[px+3]/255;dA2=(sA+dA-sA*dA);sRA=src[px]/255*sA;sGA=src[px+1]/255*sA;sBA=src[px+2]/255*sA;demultiply=255/dA2;src[px]=(sRA*dRA+dRA*(1-sA))*demultiply;src[px+1]=(sGA*dGA+dGA*(1-sA))*demultiply;src[px+2]=(sBA*dBA+dBA*(1-sA))*demultiply;}
...
ig.baked=true;ig.module('game.entities.square').requires('impact.entity').defines(function(){var COLORS,MAX_VEL,MIN_HEIGHT,MIN_VEL,MIN_WIDTH,PEERS_SPAWNED,SECS_UNTIL_ACTIVE;PEERS_SPAWNED=1;MIN_WIDTH=16;MIN_HEIGHT=16;MIN_VEL=20;MAX_VEL=100;COLORS=['red','blue','yellow','white','beige'];SECS_UNTIL_ACTIVE=1;return window.EntitySquare=ig.Entity.extend({init:function(x,y,settings){this.parent(x,y,settings);this.settings=settings;this.initWidthHeight();this.initColor();this.initVels();this.type=ig.Entity.TYPE.B;this.spawn_timer=new ig.Timer(SECS_UNTIL_ACTIVE);this.active=false;return this.currentAnim.alpha=0;},update:function(){if(this.touchingScreenEdges().length>0){this.bounceOffEdge();}
this.parent();if(this.spawn_timer!=null){return this.checkSpawnTimer();}},checkSpawnTimer:function(){if(this.spawn_timer.delta()>0){this.spawn_timer=null;this.active=true;return this.currentAnim.alpha=0.9;}else{return this.currentAnim.alpha=1+this.spawn_timer.delta();}},spawnPeers:function(player){var i,x,x_offset,y,y_offset,_i,_results;_results=[];for(i=_i=0;0<=PEERS_SPAWNED?_i<=PEERS_SPAWNED:_i>=PEERS_SPAWNED;i=0<=PEERS_SPAWNED?++_i:--_i){x_offset=getRandomInt(this.width,this.width*2);if(Math.random()<0.5){x_offset*=-1;}
y_offset=getRandomInt(this.height,this.height*2);if(Math.random()<0.5){y_offset*=-1;}
x=this.pos.x+(this.width/2)+x_offset;y=this.pos.y+(this.height/2)+y_offset;_results.push(this.settings.game.spawnEntity(EntitySquare,x,y,{game:this.settings.game,color:this.color}));}
return _results;},"switch":function(color){return this.updateColor(color);},getAnimSheetForColor:function(color){return new ig.AnimationSheet('media/white_320x240.png#'+this.hexForColor(color),this.width,this.height);},randomColor:function(){return COLORS[window.getRandomInt(0,COLORS.length-1)];},initWidthHeight:function(){this.width=this.settings.width||window.getRandomInt(MIN_WIDTH,ig.system.width/4);this.height=this.settings.height||window.getRandomInt(MIN_HEIGHT,ig.system.height/4);return this.size={x:this.width,y:this.height};},initColor:function(){this.color=this.settings.color||this.randomColor();return this.updateColor();},updateColor:function(color){if(color){this.color=color;}
this.animSheet=this.getAnimSheetForColor(this.color);return this.currentAnim=new ig.Animation(this.animSheet,0.1,[0],true);},initVels:function(){this.vel={x:this.maybeNegative(window.getRandomInt(MIN_VEL,MAX_VEL)),y:this.maybeNegative(window.getRandomInt(MIN_VEL,MAX_VEL))};return this.maxVel=this.vel;},maybeNegative:function(val){if(Math.random()<0.5){return val;}else{return val*-1;}},bounceOffEdge:function(){if(this.isTouching('left')||this.isTouching('right')){this.vel.x*=-1;}
if(this.isTouching('top')||this.isTouching('bottom')){return this.vel.y*=-1;}},touchingScreenEdges:function(){var touching;touching=[];if(this.pos.x<=0){touching.push('left');}
if(this.pos.y<=this.settings.game.hud.height){touching.push('top');}
if(this.pos.x+this.width>=ig.system.width){touching.push('right');}
if(this.pos.y+this.height>=ig.system.height){touching.push('bottom');}
return touching;},isTouching:function(dir){return this.touchingScreenEdges().indexOf(dir)>=0;},hexForColor:function(color){return{'red':'fd3333','blue':'072f94','yellow':'ffff3d','white':'ffffff','black':'0c0a0a','beige':'908f5f'}[color];}});});

// lib/game/entities/player.js
ig.baked=true;ig.module('game.entities.player').requires('impact.entity','game.entities.square').defines(function(){return window.EntityPlayer=EntitySquare.extend({is_player:true,sounds:{collect:new ig.Sound('media/collect.ogg'),hit:new ig.Sound('media/hit.ogg'),die:new ig.Sound('media/die2.ogg'),"switch":new ig.Sound('media/switch.ogg')},init:function(x,y,settings){this.parent(x,y,settings);this.settings=settings;this.game=this.settings.game;this.initWidthHeight();this.initColor();this.initChecks();this.speed=200;this.maxVel={x:this.speed,y:this.speed};return this.active=true;},reset:function(){this.score=0;return this.lives=3;},update:function(){return this.parent();},initChecks:function(){this.type=ig.Entity.TYPE.A;return this.checkAgainst=ig.Entity.TYPE.B;},check:function(other){var friendly_collision;if(!other){return;}
if(other.color==null){return;}
friendly_collision=this.color===other.color;if(!friendly_collision){if(other.active){other.kill();return this.onHit();}}else{other.kill();return this.onCollect();}},"switch":function(color){return this.onSwitch(color);},bounceOffEdge:function(){if(this.touchingScreenEdges().length>0){if(this.isTouching('left')||this.isTouching('right')){this.pos.x=this.last.x;}
if(this.isTouching('top')||this.isTouching('bottom')){return this.pos.y=this.last.y;}}},onSwitch:function(color){this.updateColor(color);return this.sounds["switch"].play();},onCollect:function(){this.score+=1;this.sounds.collect.play();if(this.game.noMoreOfColor(this.color)){return this["switch"](this.game.getMostPopularColor());}},onHit:function(){if(this.lives>0){this.lives-=1;}
this.sounds.hit.play();if(this.lives<=0){return this.onDie({score:this.score});}},onDie:function(args){this.sounds.die.play();return this.settings.dieCallback(args);}});});

// lib/game/entities/hud.js
ig.baked=true;ig.module('game.entities.hud').requires('impact.entity').defines(function(){var PADDING;PADDING=20;return window.EntityHud=ig.Entity.extend({font:new ig.Font("media/fonts/existence_light_16.png"),init:function(settings){this.parent(settings);this.settings=settings;this.initWidthHeight();this.player=this.settings.player;this.score_text="";return this.lives_text="";},update:function(){this.parent();this.score_text="Score: "+this.player.score;return this.lives_text="Lives: "+this.player.lives;},draw:function(){this.font.draw(this.score_text,PADDING,PADDING);return this.font.draw(this.lives_text,ig.system.width-PADDING,PADDING,ig.Font.ALIGN.RIGHT);},addAnims:function(){var anim,color,colors,frame,_i,_len,_results;colors=['red','green','blue','yellow','white'];_results=[];for(frame=_i=0,_len=colors.length;_i<_len;frame=++_i){color=colors[frame];anim=this.addAnim(color,99,[0]);_results.push(anim.color=color);}
return _results;},getAnimSheetForColor:function(color){return new ig.AnimationSheet('media/white_320x240.png#'+this.hexForColor(color),this.width,this.height);},initWidthHeight:function(){this.height=this.font.height+PADDING;this.width=ig.system.width;this.size={x:this.width,y:this.height};this.pos.x=0;return this.pos.y=0;}});});

// lib/game/play_scene.js
ig.baked=true;ig.module("game.play_scene").requires("impact.game","impact.font","plugins.scene_manager","game.imageblender","game.entities.square","game.entities.player","game.entities.hud").defines(function(){var ENTITY_SIZE,INITIAL_SPAWN_TIME,MAX_OTHERS;ENTITY_SIZE=8;INITIAL_SPAWN_TIME=2;MAX_OTHERS=12;return window.PlayScene=Scene.extend({font:new ig.Font("media/04b03.font.png"),init:function(manager){this.manager=manager;this.bindKeys();this.spawn_others_timer=new ig.Timer(INITIAL_SPAWN_TIME);return window.Game=this;},reset:function(){var _this=this;this.player=this.spawnEntity(EntityPlayer,ig.system.width/2,ig.system.height/2,{width:8,height:8,color:'red',game:this,dieCallback:(function(args){return _this.gameOver(args);})});this.player.reset();this.hud=this.spawnEntity(EntityHud,{player:this.player});return this.field={width:ig.system.width,height:ig.system.height-this.hud.height-10};},update:function(){this.parent();this.updatePlayerXY();return this.checkAndSpawnOthers();},draw:function(){return this.parent();},bindKeys:function(){ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');return ig.input.bind(ig.KEY.SPACE,'switch');},checkAndSpawnOthers:function(){if(!this.isTimeToSpawn()){return;}
this.spawn_others_timer.set(this.getNextSpawnTime());if(this.noMoreOfColor(this.player.color)){return this.spawnSquare({color:this.player.color});}
if(this.isRoomForMore()){return this.spawnSquare();}},isTimeToSpawn:function(){return this.spawn_others_timer.delta()>0;},isRoomForMore:function(){return this.getEntitiesByType(EntitySquare).length-1<MAX_OTHERS;},getNextSpawnTime:function(){if(this.player.score<=0){return INITIAL_SPAWN_TIME;}else{return INITIAL_SPAWN_TIME/(this.player.score/3);}},spawnSquare:function(args){var padding,x,y;if(args==null){args={};}
padding=100;x=getRandomInt(padding,ig.system.width-ENTITY_SIZE-padding);y=getRandomInt(this.hud.height+padding,ig.system.height-ENTITY_SIZE-padding);return this.spawnEntity(EntitySquare,x,y,{game:this,color:args.color});},gameOver:function(die_callback_args){var ent,_i,_len,_ref;_ref=this.entities;for(_i=0,_len=_ref.length;_i<_len;_i++){ent=_ref[_i];ent.kill();}
return this.manager.endGame({score:die_callback_args.score});},updatePlayerXY:function(){this.player.vel.x=0;this.player.vel.y=0;if(ig.input.state('left')){this.player.vel.x=-1*this.player.speed;}
if(ig.input.state('right')){this.player.vel.x=this.player.speed;}
if(ig.input.state('up')){this.player.vel.y=-1*this.player.speed;}
if(ig.input.state('down')){this.player.vel.y=this.player.speed;}
if(ig.input.pressed('switch')){return this.player["switch"](this.getMostPopularColor());}},getMostPopularColor:function(){var color,count,highest_count,most_popular_color,_ref;highest_count=0;most_popular_color=null;_ref=this.getColorCounts();for(color in _ref){count=_ref[color];if(count>highest_count){highest_count=count;most_popular_color=color;}}
return most_popular_color;},getColorCounts:function(){var color_counts,square,squares,_i,_len;squares=this.getEntitiesByType(EntitySquare);color_counts={};for(_i=0,_len=squares.length;_i<_len;_i++){square=squares[_i];if(color_counts[square.color]!=null){color_counts[square.color]+=1;}else{color_counts[square.color]=1;}}
color_counts[this.player.color]-=1;return color_counts;},noMoreOfColor:function(color){var counts;counts=this.getColorCounts();return(counts[color]==null)||counts[color]===0;}});});

// lib/game/over_scene.js
ig.baked=true;ig.module("game.over_scene").requires("impact.game","impact.font","plugins.scene_manager").defines(function(){return window.OverScene=Scene.extend({h1_font:new ig.Font("media/fonts/existence_light_42_2.png"),h2_font:new ig.Font("media/fonts/existence_light_32.png"),font:new ig.Font("media/fonts/existence_light_16.png"),init:function(manager){this.manager=manager;return this.bindKeys();},bindKeys:function(){return ig.input.bind(ig.KEY.ENTER,'start');},update:function(){this.parent();if(ig.input.pressed('start')){return this.manager.startGame();}},draw:function(){var center_x,center_y,credits_text,engine_text,text,thanks_text,top_y;this.parent();center_x=ig.system.width/2;center_y=ig.system.height/2;top_y=40;text="Game Over\n\n";text="Score: "+this.score+"\n";this.h1_font.draw(text,center_x,top_y,ig.Font.ALIGN.CENTER);text="Press Enter to retry...";this.h2_font.draw(text,center_x,top_y+50,ig.Font.ALIGN.CENTER);credits_text="Created by Gabe Hollombe (@gabehollombe)\n";credits_text+="For Ludum Dare #26, April 26-28 2013\n";this.font.draw(credits_text,center_x,top_y+140,ig.Font.ALIGN.CENTER);engine_text="Made with Impact, Bfxr, Pixen,\nAudacity, Garage Band,\nChrome, and Vim";this.font.draw(engine_text,30,ig.system.height-200);thanks_text="Special Thanks\n";thanks_text+="Alexandra - Initial concept feedback\n";thanks_text+="Cameron and Helena - Sunday night dinner\n";thanks_text+="The Ludum Dare community - Encouragement and support\n";return this.font.draw(thanks_text,30,ig.system.height-110,ig.Font.ALIGN.LEFT);},setScore:function(score){return this.score=score;}});});

// lib/game/main.js
ig.baked=true;ig.module("game.main").requires("impact.game","impact.font","plugins.scene_manager","game.menu_scene","game.play_scene","game.over_scene").defines(function(){var ParentGame;ParentGame=ig.Game.extend({sceneManager:new SceneManager(),main_music_track:new ig.Sound('media/music/theme.ogg',false),init:function(){ig.soundManager.volume=0.1;this.menu_scene=new MenuScene(this);this.over_scene=new OverScene(this);this.sceneManager.replaceScene(this.menu_scene);if(typeof NO_MUSIC==="undefined"||NO_MUSIC===null){return this.initMusic();}},update:function(){return this.sceneManager.updateScene();},draw:function(){return this.sceneManager.drawScene();},startGame:function(){this.play_scene=new PlayScene(this);this.play_scene.reset();this.sceneManager.replaceScene(this.play_scene);return ig.music.play();},endGame:function(args){this.over_scene.setScore(args.score);this.sceneManager.replaceScene(this.over_scene);return ig.music.stop();},initMusic:function(){ig.music.add(this.main_music_track);return ig.music.volume=0.5;}});return ig.main("#canvas",ParentGame,60,640,480,1);});